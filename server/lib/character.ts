...
export default class Character {
    ...
    static sanitizeMetadata(character: ICharacter, json: any): void {
    ...
    static sanitizeTimestamp(character: ICharacter): void {
    ...
    static ReadFromFile(filepath: string): ICharacter | null {
        ...
        try {
            ...
            if (!json.spec || !json.data) {
                character = this.parseCharacterV1(json);
            }else{
                character = json;
            }
            ...
            if (character) {
                ...
                this.sanitizeMetadata(character, json);
                this.sanitizeTimestamp(character);
                return character
            }
    ...
    static async WriteToFile(character: Character, filepath: string, buffer: Buffer | null): Promise<boolean> {
        try {
            if (!buffer) {
